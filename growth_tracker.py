...
class GrowthCalculator:
    ...
    def detect_growth_trends(self, measurements, measurement_type):
        """Detect significant growth trends and percentile crossings"""
        if len(measurements) < 2:
            return "Insufficient data for trend analysis"
        
        type_measurements = [m for m in measurements if m['type'] == measurement_type]
        type_measurements.sort(key=lambda x: x['age_months'])
        
        if len(type_measurements) < 2:
            return "Insufficient measurements for trend analysis"
        
        first = type_measurements[0]
        last = type_measurements[-1]
        
        if first.get('percentile') and last.get('percentile'):
            percentile_change = last['percentile'] - first['percentile']
            
            if percentile_change < -20:
                return f"Significant decrease in percentile ({percentile_change:.1f}%) - Clinical concern"
            elif percentile_change < -10:
                return f"Moderate decrease in percentile ({percentile_change:.1f}%) - Monitor closely"
            elif percentile_change > 20:
                return f"Significant increase in percentile ({percentile_change:.1f}%) - May warrant evaluation"
            elif percentile_change > 10:
                return f"Moderate increase in percentile ({percentile_change:.1f}%) - Monitor"
            else:
                return "Stable growth pattern"
...
class PDFReportGenerator:
    ...
    def create_comprehensive_report(self, patient_info, measurements, charts):
        ...
        try:
            ...
            # Growth Trends
            story.append(Paragraph("GROWTH TRENDS", heading_style))
            trend_analysis = self._generate_trend_analysis(measurements)
            story.append(Paragraph(trend_analysis, styles['Normal']))
            story.append(Spacer(1, 0.2*inch))
    ...
    def _generate_trend_analysis(self, measurements):
        """Generate trend analysis based on multiple measurements"""
        if len(measurements) < 2:
            return "Insufficient data for trend analysis. Additional measurements needed."
        
        trends = []
        for measurement_type in ['weight_age', 'height_age', 'head_age', 'bmi_age']:
            trend = self.calculator.detect_growth_trends(measurements, measurement_type)
            if "Insufficient" not in trend:
                display_name = self._get_display_name(measurement_type)
                trends.append(f"{display_name}: {trend}")
        
        if trends:
            return " | ".join(trends)
        else:
            return "No significant trends detected in available measurements."
    ...
    def _generate_clinical_assessment(self, latest_measurements, all_measurements):
        ...
        trend_analysis = self._generate_trend_analysis(all_measurements)
        if "decrease" in trend_analysis.lower() or "concern" in trend_analysis.lower():
            concerns.append(f"Trend analysis: {trend_analysis}")
    ...
    def _generate_recommendations(self, latest_measurements, all_measurements):
        ...
        trend_analysis = self._generate_trend_analysis(all_measurements)
        if "decrease" in trend_analysis.lower():
            recommendations.append("• Schedule follow-up measurement in 1-2 months to monitor trends")
        else:
...
def process_measurement_submission(first_name, last_name, gender, birth_date, measurement_date, 
    ...
    if new_measurements:
        ...
        if len(st.session_state.measurements) > 3:
            st.markdown("---")
            st.subheader("Growth Trends")
            for measurement_type in ['weight_age', 'height_age']:
                trend = calculator.detect_growth_trends(st.session_state.measurements, measurement_type)
                if "decrease" in trend.lower():
                    st.markdown(f'<div class="trend-alert">⚠️ {trend}</div>', unsafe_allow_html=True)
                elif "Insufficient" not in trend:
                    st.write(f"**{measurement_type.replace('_', ' ').title()}:** {trend}")
    else:
